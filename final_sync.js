...
files.forEach(file => {
    ...
    // Clean up any extra announcement bars or trust bars if they are outside <header> (reverting to commit e25c36b might have left them)
    // Actually, checking commit e25c36b index.html structure earlier:
    // It had announcement bar INSIDE <header>. 
    // Wait, the diff showed e25c36b added .header-top etc.
    // If they were outside, I should remove them.
    content = content.replace(/<div class="header-top">[\s\S]*?<\/div>/i, '');
    content = content.replace(/<div class="header-trust-bar">[\s\S]*?<\/div>/i, '');
    content = content.replace(/<div class="back-to-top"[\s\S]*?<\/div>/i, '');
