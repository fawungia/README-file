...
postFiles.forEach(file => {
    ...
    // 1. Extract Header & Footer (UNTOCUCHED as requested)
    const headerMatch = content.match(/<header>([\s\S]*?)<\/header>/i);
    const header = headerMatch ? headerMatch[0] : '';
    const footerMatch = content.match(/<footer>([\s\S]*?)<\/footer>/i);
    const footer = footerMatch ? footerMatch[0] : '';
    
    // 2. Extract Final Script (UNTOUCHED)
    const scriptMatch = content.match(/<script[^>]*>([\s\S]*?)<\/script>\s*<\/body>/i);
    const finalScript = scriptMatch ? scriptMatch[0].replace('</body>', '') : '<script src="script.js"></script>';

    // 3. Extract Unique Article Data
    const title = extract(content, /<title>([\s\S]*?)<\/title>/i);
    const description = extract(content, /<meta name="description" content="([\s\S]*?)">/i);
    const pVerify = extract(content, /<meta name="p:domain_verify" content="([\s\S]*?)"\/>/i);

    // Extract Headline & Category
    const headline = extract(content, /<h1[^>]*>([\s\S]*?)<\/h1>/i);
    let category = extract(content, /<span class="cat-tag">([\s\S]*?)<\/span>/i);
    if (!category) {
        // Fallback: look for category in my messy nested divs from previous script
        const metaMatch = content.match(/<span class="article-meta">([\s\S]*?)<\/span>/i);
        if (metaMatch) {
    ...
    // This is the hardest part. I'll look for everything inside .article-content that ISN'T a shop-callout.
    let articleInner = '';
    ...
    if (bodyMatch) {
        // Clean out my previous script's extra wrappers if they exist
        articleInner = bodyMatch[1].trim()
            .replace(/<div class="article-body">/g, '')
            .replace(/<div class="article-content[^>]*">/g, '')
            .replace(/<\/div>/g, '')
            .replace(/<\/article>/g, '');
    }

    // Extract Sidebar Shop Items
    let shopItems = '';
    const shopWidgetMatch = content.match(/<h3 class="widget-title">Shop This Post<\/h3>([\s\S]*?)<\/div>\s*<\/div>/i);
    if (shopWidgetMatch) {
        shopItems = shopWidgetMatch[1].trim();
    }

    // 4. CONSTRUCT FINAL PAGE
...
<head>
    ...
    <meta name="p:domain_verify" content="${pVerify}"/>
