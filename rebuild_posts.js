...
Object.keys(postsData).forEach(file => {
    ...
    // Extract Header (UNTOCUCHED)
    const headerMatch = oldContent.match(/<header>([\s\S]*?)<\/header>/i);
    const header = headerMatch ? headerMatch[0] : '';

    // Extract Footer (UNTOUCHED)
    const footerMatch = oldContent.match(/<footer>([\s\S]*?)<\/footer>/i);
    const footer = footerMatch ? footerMatch[0] : '';
    
    // Extract Script (UNTOUCHED)
    const scriptMatch = oldContent.match(/<script[^>]*>([\s\S]*?)<\/script>\s*<\/body>/i);
...
${footer}
    ...
    console.log(`Rebuilt ${file} with clean master template.`);
});
